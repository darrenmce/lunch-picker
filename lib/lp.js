...
var _addItem = function (name, weight) {
  return low(type).insert(_createItem(name, weight)).value();
};
...
var _updateLuck = function (winnerId) {
  ...
  var totalWeight = low(type).reduce(function (sum, next) {
    return sum + parseFloat(next.weight);
  }, 0);
...
var _luckCalc = function (totalWeight, item, won) {
  ...
  var luck = parseFloat(item.luckFactor) + (adjustment * luckMovementFactor);

  return Math.max(minLuck,luck);
};
